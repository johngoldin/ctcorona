...
manual_supplement <- tibble(
  ...
  county = rep(c("Fairfield", "Hartford", "Litchfield", "Middlesex", "New Haven", "New London", "Tolland", "Windham", "Unknown"), 13),
...
c(rep(week_setup$start_period, 7))

ct3 <- ct2 %>% mutate(county = fct_collapse(county, Fairfield = "Fairfield", 
                                             `New Haven` = "New Haven",
                                            Hartford = "Hartford", group_other = TRUE),
                      ) %>% 
  group_by(county, date) %>% 
  summarise(cases = sum(cases), deaths = sum(deaths), hospital = sum(hospital))

ct_recent <- ct3 %>% filter(date >= (max(ct3$date) - 6))
fairfield <- ct_recent %>% filter(county == "Fairfield") %>% 
  mutate(d =  as.numeric(date - min(ct_recent$date)) )
fairfield2 <- ct3 %>% filter(county == "Fairfield",
  date >= (max(ct3$date) - 13), date < (max(ct3$date) - 6)) %>% 
  mutate(d = as.numeric(date - min(ct_recent$date)))
lm(log(cases) ~ d, data = fairfield)
lm(log(cases) ~ d, data = fairfield2)
library(broom)
slopes <- ct_recent %>%
  mutate( d = as.numeric(date - min(ct_recent$date))) %>% 
  nest(-county, d, cases) %>% 

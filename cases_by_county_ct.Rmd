...
manual_supplement <- tibble(
  ...
  county = rep(c("Fairfield", "Hartford", "Litchfield", "Middlesex", "New Haven", "New London", "Tolland", "Windham", "Unknown"), 14),
...
week_setup <- tibble(
  ...
  period_label = str_c(str_sub(start_period, 6, 10), " to ", str_sub(end_period, 6, 10)),
  period_label = str_replace_all(period_label, "-", "/"))
ct <- ct2 %>% 
...
doubling_cases <- ct3 %>% 
  filter(!is.na(period_label), cases > 0) %>% 
  group_by(county, period_label) %>% 
  nest() %>% 
  mutate(
  ...
  unnest(tidied_cases) %>% 
  select(county, period_label, data, fit_cases, term, estimate) %>% 
  pivot_wider(values_from = estimate, names_from = term) %>% 
  mutate(doubling = log(2) / log(1 + date))

doubling_deaths <- ct3 %>% 
  filter(!is.na(period_label), deaths > 0) %>% 
  group_by(county, period_label) %>% 
  nest() %>% 
  mutate(
  ...
  select(county, period_label, data, fit_deaths, term, estimate) %>% 
  pivot_wider(values_from = estimate, names_from = term) %>% 
  mutate(doubling = log(2) / log(1 + date))
doubling <-
  bind_rows(
    doubling_cases %>% 
      select(county, week = period_label, daily_rate = date, doubling) %>% 
...
p <- ggplot(data = doubling %>% filter(measure == "cases"), aes(x = week, y = doubling, fill = doubling)) + 
...
# pcases + 
#   geom_smooth(data = ct3 %>% filter(period_label == "04/02 to 04/08"),
#               aes(y = exp(cases)),
#               method = "lm", se = FALSE)
```
...
pcases + geom_smooth(data = ct %>% filter(date > ymd("2020-03-27")), 
                                          aes(group = period_label, colour = county),
                   method = "lm", se = FALSE)
```

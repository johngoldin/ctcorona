...
I think there
may be an issue with Mansfield and prisons. I don't know what's going on with Simsbury.
...
```
```{r gender_from_census2, echo = FALSE, message = FALSE}
vars2020 <- load_variables(2020, "pl", cache = TRUE) 
if (1 == 2) {
  sex_age_acs2 <- get_acs_age_sex(acs_vars = vars)
  sex_age_acs3 <- sex_age_acs2 |> 
    group_by(TOWN, age) |> 
  ...
  sex_age_acs2014_3 <- sex_age_acs2014_2 |> 
    group_by(TOWN, age) |> 
    summarise(estimate = sum(estimate))
  save(sex_age_acs2, sex_age_acs2014_2, file = "~/Documents/R_local_repos/ctcorona/data/census_acs_data.RData")
}
...
```{r pop_compre_plot, echo = FALSE, message = FALSE}

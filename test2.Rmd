...
county_sparklines <- dph_counties %>% 
  ...
  summarise(`county cases` = spk_chr(rnew_cases),
            `county deaths` = spk_chr(rnew_deaths))
test <- dph_towns %>% 
  ...
  summarise(`max cases` = round(max(rnew_cases, na.rm = TRUE)),
            `cases trend` = spk_chr(rnew_cases)) #%>% 
  left_join(town_info %>% select(town, county, category, total_pop), by = "town")  %>%
  left_join(county_sparklines, by = "county") %>% 
  select(town, county, `cases trend`)
test2 <- test %>% 
  formattable::format_table(
    # x = .,

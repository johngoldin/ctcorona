...
test <- test %>% 
  ...
  summarise(`max cases` = (max(rnew_cases_per1k, na.rm = TRUE) %>% round(4)),
    `town cases` = spk_chr(rnew_cases, chartRangeMin = 0, 
                           chartRangeMax = max(rnew_cases_per1k, na.rm = TRUE)),
    `town deaths` = spk_chr(rnew_deaths, chartRangeMin = 0, 
                           chartRangeMax = max(rnew_deaths_per1k, na.rm = TRUE)), .groups = "drop") %>% 

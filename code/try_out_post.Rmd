...
```
```{r kdkd, echo = FALSE}
p <- ggplot(data = town_geometries %>% filter(county.y == "New Haven"), aes(geometry = geometry)) +
  geom_sf() +
   geom_sf(# need aes(geometry = geometry) because joined sf to a tibble and so lost sf
          aes(fill = total_pop, geometry = geometry)) +
  # scale_fill_gradient(low = "white", high = "grey") +
  # geom_sf_text(aes(label =town, color = "black", size = 3) +
  geom_sf_text(aes(label = town), size = 3, colour = "yellow") +
  coord_sf(datum = NA, label_axes = "----") +
  xlab("") + ylab("") + theme_minimal()  +
  # theme(legend.position = "none") +

...
the total number of cases, the number of recent cases per
100K of population, total deaths, and percentage of total deaths
in that town
...
town_summary <- dph_towns %>%
  ...
  left_join(towns_recent_week %>% mutate(recent_rate = cases / (total_pop / 100000)) %>% 
              select(town, recent_rate)) %>% 
  left_join(town_with_nursing %>% 
              filter(date == max(date)) %>% 
              select(town, nh_death_pct), by = "town") %>% 
  ...
  select(town, category, total_pop, `total cases`, recent_rate, 
         `total deaths`, nh_death_pct, `town cases`, `town deaths`) %>% 
  formattable::format_table(
    # x = .,
    # formatters = list(align=c("l")),
    format = "html",

...
xx <- dph_total %>% select(date, rnew_cases, new_cases)  %>%  pivot_longer(cols = c("rnew_cases", "new_cases"), names_to = "cases") %>% 
  full_join(
dph_total %>% select(date, rnew_deaths, new_deaths)  %>%  pivot_longer(cols = c("rnew_deaths", "new_deaths"), names_to = "deaths"), by = "date"  )
```
